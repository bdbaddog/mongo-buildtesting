...
then
    #MacOS
    flags="-j${cores} --link-model=dynamic --implicit-cache --disable-warnings-as-errors --cache --build-fast-and-loose=on --modules= --variables-files=etc/scons/xcode_macosx.varsg --cache-debug=- --debug=explain"
else
    # Linux
...
cp -v ../mongo_ssd/${path_to_nodes}/{__init__.py,FS.py} ./${path_to_nodes}/

git checkout r3.7.1
/usr/bin/time python ${profile_flags}  ./buildscripts/scons.py ${flags} ${target} 2>&1 | tee  ${log_dir}/BUILD.log.371.${revision}.1
...
git checkout r3.7.2
/usr/bin/time python ${profile_flags} ./buildscripts/scons.py ${flags} ${target} 2>&1 | tee  ${log_dir}/BUILD.log.372.${revision}.1
...
git checkout r3.7.1
/usr/bin/time python ${profile_flags} ./buildscripts/scons.py ${flags} ${target} 2>&1 | tee  ${log_dir}/BUILD.log.371.${revision}.2
