...
git checkout r3.7.1
/usr/bin/time python -m cProfile -o profile_data_371_1.pyprof  ./buildscripts/scons.py ${flags} ${target} 2>&1 | tee  ${log_dir}/BUILD.log.371.${revision}.1
...
git checkout r3.7.2
/usr/bin/time python -m cProfile -o profile_data_372_1.pyprof ./buildscripts/scons.py ${flags} ${target} 2>&1 | tee  ${log_dir}/BUILD.log.372.${revision}.1
...
git checkout r3.7.1
/usr/bin/time python -m cProfile -o profile_data_371_2.pyprof ./buildscripts/scons.py ${flags} ${target} 2>&1 | tee  ${log_dir}/BUILD.log.371.${revision}.2
